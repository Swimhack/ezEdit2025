#!/bin/bash
# EXACT COMMANDS FOR DIGITALOCEAN WEB CONSOLE DEPLOYMENT
# Copy and paste these commands into the DigitalOcean console

# Step 1: Navigate to web directory and backup current files
cd /var/www/html
mkdir -p /backup/$(date +%Y%m%d_%H%M%S)
cp -r * /backup/$(date +%Y%m%d_%H%M%S)/ 2>/dev/null || true

# Step 2: Clear current directory (keeping backups safe)
rm -rf /var/www/html/*

# Step 3: Create the deployment structure
...
# Step 5: Deploy CSS files
cat > /var/www/html/css/main.css << 'EOFMAINCSS'
:root {
    --primary: #2563eb;
    --primary-dark: #1d4ed8;
    --secondary: #64748b;
...
# Step 6: Deploy JavaScript
cat > /var/www/html/js/main.js << 'EOFMAINJS'
