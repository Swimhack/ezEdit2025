# 🚀 Deploy EzEdit.co with Claude Code Integration

## **Quick Deploy to DigitalOcean**

Copy and paste these commands into your DigitalOcean console:
...
### **Step 2: Deploy Updated Homepage**
```bash
cat > /var/www/html/index.php << 'HOMEPAGE_EOF'
<?php session_start(); ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
### **Step 3: Create API Directory and Claude Code Backend**
```bash
mkdir -p /var/www/html/api && cat > /var/www/html/api/ai-assistant.php << 'API_EOF'
<?php
...
### **Step 4: Deploy Updated Login System**
```bash
mkdir -p /var/www/html/auth && cat > /var/www/html/auth/login.php << 'LOGIN_EOF'
<?php
session_start();
if ($_POST) {
    $email = $_POST['email'] ?? '';
...
```bash
cat > /var/www/html/dashboard.php << 'DASHBOARD_EOF'
<?php 
session_start();
...
### **Step 6: Set Permissions and Reload**
```bash
chown -R www-data:www-data /var/www/html && chmod -R 755 /var/www/html && chmod 644 /var/www/html/api/ai-assistant.php && systemctl reload nginx
```
...
- **Claude Code Editor:** http://159.65.224.175/editor.php
- **API Test:** http://159.65.224.175/test-claude-code.html

**Login Credentials:** `test@example.com` / `password`

## **✅ What's Deployed**

- ✅ Updated homepage with Claude Code branding
- ✅ Functional Claude Code AI assistant API
- ✅ Professional three-pane editor interface
- ✅ Real-time AI code analysis and suggestions
- ✅ Security scanning and optimization tools
- ✅ FTP integration for legacy website editing
- ✅ Complete user authentication system

**Your site now has a fully functional Claude Code AI assistant!** 🚀